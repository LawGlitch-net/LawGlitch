...
function startRoomLinkPolling() {
  ...
  window.roomLinkPollingInterval = setInterval(async () => {
    console.log("DEBUG: Polling for room link and checking meetingid...");
    try {
      if (!supabaseClient) {
      ...
      // Check if there's text in the meetingid input
      const meetingIdInput = document.getElementById('meetingid');
      if (meetingIdInput && meetingIdInput.value) {
        console.log('DEBUG: Found meetingid with value:', meetingIdInput.value);
        
        // Try to get booking ID from parent window
        let bookingId = null;
        try {
          const parentBookingInput = window.parent.document.getElementById('booking-id');
          if (parentBookingInput && parentBookingInput.value) {
            bookingId = parentBookingInput.value;
            console.log('DEBUG: Found booking ID from parent:', bookingId);
          }
        } catch (e) {
          console.warn('Could not access parent window booking ID:', e);
        }

        // If we have both meetingid and bookingId, update Supabase
        if (bookingId) {
          console.log('DEBUG: Attempting to update Supabase with room_link:', meetingIdInput.value, 'for booking:', bookingId);
          const { data: updated, error: updateError } = await supabaseClient
            .from('bookings')
            .update({ room_link: meetingIdInput.value })
            .eq('id', bookingId)
            .select();
          ...
          if (updateError) {
            ...
            console.log('Successfully updated booking with room_link:', updated);
          }
        } else {
          console.log('DEBUG: No booking ID found in parent window, skipping Supabase update');
        }
      } else {
        console.log('DEBUG: No meetingid value found yet');
      }
...
async function createNewMeeting() {
  console.log("DEBUG: createNewMeeting function called.");
  try {
    console.log("Creating new meeting...");
    
    // Get token from worker
    const generatedToken = await tokenGeneration();
    console.log("DEBUG: Token generation result:", generatedToken);
    ...
    console.log("DEBUG: Creating meeting with token:", token ? "present" : "missing");
    
    if (!CREATE_MEETING_URL) {
      throw new Error("CREATE_MEETING_URL is not defined. Make sure config.js is loaded properly.");
    }

    const createMeetingBody = bookingId ? { bookingId } : {};
    console.log("DEBUG: Creating meeting with body:", createMeetingBody);
    ...
    console.log("DEBUG: Create meeting response status:", response.status);
    ...
    try {
      console.log('DEBUG: Attempting to update Supabase with room_link. displayedId:', displayedId, 'finalBookingId:', finalBookingId);
...
async function tokenGeneration() {
  ...
  try {
    console.log("DEBUG: Attempting to get token from:", AUTH_URL);
    if (!AUTH_URL) {
    ...
    const data = await response.json();
    console.log("DEBUG: Token data received:", data ? "data present" : "no data");
    ...
    alert("Failed to get authentication token. Please try again. Error: " + error.message);
    return null;
  }
}
...
window.validateMeeting = validateMeeting;
window.startRoomLinkPolling = startRoomLinkPolling;