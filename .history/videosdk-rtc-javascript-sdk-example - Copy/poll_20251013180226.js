...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // After initialization, attempt to detect bookingId and poll the bookings table for meeting code
    (async () => {
        try {
            const bookingId = getBookingId();
            if (bookingId) {
                console.log('🔎 Found bookingId after init, starting poll for meeting code:', bookingId);
                const result = await pollBookingForMeetingCode(bookingId, { interval: 2000, timeout: 20000 });
                if (result) {
                    console.log('🎉 Polling returned meeting code:', result);
                } else {
                    console.log('ℹ️ Polling did not find a meeting code for booking:', bookingId);
                }
            } else {
                console.log('ℹ️ No bookingId available to poll after init');
            }
        } catch (e) {
            console.error('❌ Error during post-init polling:', e);
        }
    })();
});