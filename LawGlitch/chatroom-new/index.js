// ⚠️ SAFETY CHECK: Pro view or missing elements detection
const IS_PRO_VIEW = window.ChatroomRouter && typeof window.ChatroomRouter.isProView === 'function' && window.ChatroomRouter.isProView();
const MAIN_JOIN_ELEMENT = document.getElementById("joinMeetingId");

if (IS_PRO_VIEW || !MAIN_JOIN_ELEMENT) {
  console.log('ℹ️ VideoSDK: Skipping initialization (Pro view or client interface not found)');
  // This script runs but critical parts won't execute without the required elements
}

// Constants
const API_BASE_URL = "https://api.videosdk.live";

// Token configuration - Get from config.js or use empty if not available
const TOKEN = (typeof window.config !== 'undefined' && window.config && window.config.TOKEN) ? window.config.TOKEN : "";
const AUTH_URL = (typeof window.config !== 'undefined' && window.config && window.config.AUTH_URL) ? window.config.AUTH_URL : "";

// Declaring variables
let token = ""; // This will hold the actual JWT token
...
// Only attach event listeners if elements exist (client view only)
if (joinMeetingCode) {
...
window.addEventListener("load", async function () {
  // ⚠️ Safety Guard: Skip if pro view or elements don't exist
  if (IS_PRO_VIEW || !MAIN_JOIN_ELEMENT) {
    console.log('ℹ️ Skipping VideoSDK load initialization');
    return; // Exit this event listener
  }
...
async function tokenGeneration() {
  try {
    // If TOKEN is hardcoded, use it directly
    if (TOKEN && TOKEN != "") {
      token = TOKEN;
      console.log('✅ Using hardcoded TOKEN');
      return;
    } 
    // Otherwise, fetch token from AUTH_URL (Cloudflare Worker)
    else if (AUTH_URL && AUTH_URL != "") {
      console.log('🔄 Fetching token from AUTH_URL:', AUTH_URL);
    ...
    // If neither TOKEN nor AUTH_URL is configured
    else {
      throw new Error('Neither TOKEN nor AUTH_URL is configured. Check config.js');
    }
  } catch (error) {
...
/**
 * Create a new meeting for PRO view
 * Generates a meeting code that can be shared with clients
 * Pro users can then share this code with clients to join
 */
async function createNewMeeting() {
  try {
    console.log('🎬 Creating new meeting for pro view...');
    
    // Check if we're in pro view
    if (!IS_PRO_VIEW) {
      console.warn('⚠️ createNewMeeting should only be called in pro view');
      return;
    }

    // Get the create button and disable it while processing
    const createBtn = document.getElementById('createMeetingButton');
    if (createBtn) {
      createBtn.disabled = true;
      createBtn.textContent = 'Creating meeting...';
    }
    ...
    if (!token) {
      ...
      if (createBtn) {
        ...
        createBtn.textContent = 'Create a Meeting';
      }
      return;
    }
    ...
    console.log('📡 Calling VideoSDK API to create meeting...');
    ...
    // Display the meeting code
    const meetingIdElement = document.getElementById('meetingid');
    if (meetingIdElement) {
      meetingIdElement.value = roomId;
      meetingIdElement.dispatchEvent(new Event('input', { bubbles: true }));
      console.log('📋 Meeting code set:', roomId);
    }

    // Show copy notification
    const statusElement = document.getElementById('meeting-status');
    if (statusElement) {
      statusElement.textContent = '✅ Meeting created! Code: ' + roomId + ' (Click to copy)';
      statusElement.style.color = '#44ff44';
      statusElement.style.backgroundColor = '#004400';
    ...
    if (statusElement) {
      ...
      statusElement.style.backgroundColor = '#3d0000';
      statusElement.style.padding = '10px';
      statusElement.style.borderRadius = '5px';
      statusElement.style.marginTop = '10px';
    }
