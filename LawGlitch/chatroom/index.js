...
const IS_PRO_VIEW = window.VideoSDKRouter && typeof window.VideoSDKRouter.isProView === 'function' && window.VideoSDKRouter.isProView();
...
/**
 * Create a new meeting (PRO VIEW ONLY)
 * Generates token, creates room via VideoSDK API, and starts the meeting
 */
async function createNewMeeting() {
  try {
    // Safety check: only allow in pro view
    if (!IS_PRO_VIEW) {
      console.error('❌ createNewMeeting: Not in pro view');
      alert('This action is only available for professionals');
      return;
    }
    ...
    console.log('🔑 Token generated, creating room...');

    // Create a new room via VideoSDK API
    const createRoomUrl = `${API_BASE_URL}/v2/rooms`;
    ...
    try {
      const response = await fetch(createRoomUrl, createRoomOptions);
      ...
      if (!roomResponse || !roomResponse.roomId) {
        console.error('❌ Invalid room response:', roomResponse);
        alert('Failed to create meeting. No room ID returned.');
        return;
      }

      const meetingId = roomResponse.roomId;
      console.log('✅ Room created:', meetingId);

      // Ensure RoomManager has the correct booking ID before updating
      if (window.RoomManager && window.VideoSDKRouter) {
        window.RoomManager.syncWithBooking(window.VideoSDKRouter.getBookingId());
      }

      // Store meeting ID in the input field and trigger change event
      const meetingIdField = document.getElementById('meetingid');
      meetingIdField.value = meetingId;
      meetingIdField.dispatchEvent(new Event('input', { bubbles: true }));
      meetingIdField.dispatchEvent(new Event('change', { bubbles: true }));

      // Update UI - hide join page, show grid page
      document.getElementById('joinPage').style.display = 'none';
      document.getElementById('gridPpage').style.display = 'flex';
      toggleControls();

      // Determine pro name (can be customized)
      const proName = 'Professional';

      // Start the meeting
      console.log('🚀 Starting meeting...');
      startMeeting(token, meetingId, proName);
    ...
    console.error('❌ Unexpected error in createNewMeeting:', error);
...
async function tokenGeneration() {
  if (TOKEN != "") {
    ...
    try {
      console.log(`🔐 Fetching token from: ${AUTH_URL}`);
      ...
      if (error.message.includes('Failed to fetch')) {
        console.error('💡 Likely CORS issue - check Cloudflare Worker CORS configuration');
      }
      throw error;
    }
    ...
    if (token != "") {
      ...
      try {
        ...
        if (!result || !result.roomId) {
          console.error("❌ Invalid response - no roomId:", result);
          alert("Invalid Meeting ID - please check and try again");
          return;
        }

        if (result.roomId === meetingId) {
          console.log(`✅ Meeting validated successfully: ${meetingId}`);
          ...
          // Update meetingid field and trigger change events for room.js
          const meetingIdField = document.getElementById("meetingid");
          meetingIdField.value = meetingId;
          meetingIdField.dispatchEvent(new Event('input', { bubbles: true }));
          meetingIdField.dispatchEvent(new Event('change', { bubbles: true }));
          ...
          console.error(`❌ Room ID mismatch - expected: ${meetingId}, got: ${result.roomId}`);
          alert("Invalid Meeting ID");
        }
      } catch (error) {
        console.error("❌ Validation error:", error.message);
        alert(`Failed to validate meeting: ${error.message}`);
      }
    } else {
      alert("Authentication required. Please configure your settings.");
    }
  } catch (error) {
    console.error("❌ Unexpected error in validateMeeting:", error);
